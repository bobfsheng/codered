...
const calculateSimilarities = async (stocks, positions) => {
...
useEffect(() => {
  loadModel().then(() => {
    const rankedStocks = await calculateSimilarities(stockInfo, positions);
    // Use 'rankedStocks' to update your state and render the UI.
  });
}, []);
